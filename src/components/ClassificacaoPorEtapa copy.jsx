...
const ClassificacaoPorEtapa = ({ processId = 1 }) => {
    ...
    const fetchData = async (etapa, currentFilters) => {
        const stageId = processStageMap[etapa];
        let url = `${API_ENDPOINTS.RANKING_BY_STAGE(processId, stageId)}`;

        if (currentFilters.linhaPesquisa) {
            url = API_ENDPOINTS.RANKING_BY_LINE(processId, stageId, currentFilters.linhaPesquisa);
        } else if (currentFilters.temaPesquisa) {
            url = API_ENDPOINTS.RANKING_BY_TOPIC(processId, stageId, currentFilters.temaPesquisa);
        } else if (currentFilters.status) {
            url = API_ENDPOINTS.RANKING_BY_STATUS(processId, stageId, currentFilters.status);
        }
        ...
        try {
            ...
            const processed = processApiData(json);
    ...
    useEffect(() => {
        Object.keys(processStageMap).forEach(etapa => fetchData(etapa, filters));
    }, [filters]);

    const handleFilterChange = (filterType, value) => {
        setFilters((prev) => ({
            ...prev,
            linhaPesquisa: filterType === 'linhaPesquisa' ? value : '',
            temaPesquisa: filterType === 'temaPesquisa' ? value : '',
    ...
    const uniqueOptions = (key) => {
        ...
        Object.values(data).forEach(stage => {
            stage?.forEach(item => {
                if (key === 'status') {
                    item.candidatos.forEach(c => options.add(c.status));
                } else if (key === 'linhaPesquisa') {
                    options.add(item.linhaPesquisa);
                } else if (key === 'temaPesquisa') {
                    options.add(item.temaPesquisa);
                }
            });
        });
        ...
        <div className="container mt-4">
            ...
            <div className="d-flex justify-content-between align-items-center my-3">
                ...
                <Dropdown onSelect={(e) => handleFilterChange('status', e)}>
                    ...
                    <Dropdown.Menu>
                        <Dropdown.Item eventKey="">Todos</Dropdown.Item>
                        <Dropdown.Item eventKey="Classificado">Classificado</Dropdown.Item>
                        <Dropdown.Item eventKey="Desclassificado">Desclassificado</Dropdown.Item>
                        <Dropdown.Item eventKey="Reprovado">Reprovado</Dropdown.Item>
                        <Dropdown.Item eventKey="Aprovado">Aprovado</Dropdown.Item>
                    </Dropdown.Menu>
                </Dropdown>
            ...
            >
                <Tab eventKey="preProjeto" title="Pré-Projeto">
                    {renderTable(data.preProjeto)}
                </Tab>
                <Tab eventKey="entrevista" title="Entrevista">
                    {renderTable(data.entrevista)}
                </Tab>
                <Tab eventKey="curriculo" title="Currículo">
                    {renderTable(data.curriculo)}
                </Tab>
            </Tabs>
        </div>
    );
